...
final class WorkspacePersistenceTests: XCTestCase {
    ...
    func testSessionVisibilityPersistsExactTriplesAndCloseRestoreAreIdempotentAndPrivate() async throws {
        let fixedDate = Date(timeIntervalSince1970: 1_700_000_000)
        let first = WorkspaceClosedSessionKey(
            projectID: "projectalpha",
            taskID: "taskshared",
            sessionID: "sessionshared"
        )
        let sameSessionInAnotherTask = WorkspaceClosedSessionKey(
            projectID: "projectalpha",
            taskID: "taskother",
            sessionID: "sessionshared"
        )
        ...
        let store = WorkspaceSessionVisibilityStore(home: byoriHome, now: { fixedDate })

        try await store.close(first)
        try await store.close(first)
        try await store.close(sameSessionInAnotherTask)
        try await store.close(sameTaskAndSessionInAnotherProject)

        let expected = Set([
            first,
            sameSessionInAnotherTask,
            sameTaskAndSessionInAnotherProject,
        ])
        let storedKeys = try await store.closedSessionKeys()
        XCTAssertEqual(storedKeys, expected)

        let stateURL = byoriHome.appendingPathComponent("manager/session-visibility.json")
        var raw = try readJSONObject(at: stateURL)
        let rawEntries = try XCTUnwrap(raw["closed_sessions"] as? [[String: Any]])
        XCTAssertEqual(rawEntries.count, 3)
        XCTAssertEqual(
            Set(rawEntries.compactMap { entry -> String? in
                guard let projectID = entry["project_id"] as? String,
                      let taskID = entry["task_id"] as? String,
                      let sessionID = entry["session_id"] as? String else {
                    return nil
                }
                return "\(projectID)|\(taskID)|\(sessionID)"
            }),
            Set([
                "projectalpha|taskshared|sessionshared",
                "projectalpha|taskother|sessionshared",
                "projectbeta|taskshared|sessionshared",
            ])
        )
        XCTAssertTrue(rawEntries.allSatisfy { $0["closed_at"] as? String == "2023-11-14T22:13:20.000Z" })

        let filePermissions = try XCTUnwrap(
            FileManager.default.attributesOfItem(atPath: stateURL.path)[.posixPermissions] as? NSNumber
        )
        let managerDirectoryPermissions = try XCTUnwrap(
            FileManager.default.attributesOfItem(atPath: stateURL.deletingLastPathComponent().path)[.posixPermissions]
                as? NSNumber
        )
        let lockURL = byoriHome.appendingPathComponent("locks/session-visibility.lock")
        let lockFilePermissions = try XCTUnwrap(
            FileManager.default.attributesOfItem(atPath: lockURL.path)[.posixPermissions] as? NSNumber
        )
        let lockDirectoryPermissions = try XCTUnwrap(
            FileManager.default.attributesOfItem(atPath: lockURL.deletingLastPathComponent().path)[.posixPermissions]
                as? NSNumber
        )
        XCTAssertEqual(filePermissions.intValue & 0o777, 0o600)
        XCTAssertEqual(managerDirectoryPermissions.intValue & 0o777, 0o700)
        XCTAssertEqual(lockFilePermissions.intValue & 0o777, 0o600)
        XCTAssertEqual(lockDirectoryPermissions.intValue & 0o777, 0o700)

        let reloaded = WorkspaceSessionVisibilityStore(home: byoriHome)
        let reloadedKeys = try await reloaded.closedSessionKeys()
        XCTAssertEqual(reloadedKeys, expected)

        try await reloaded.restore(first)
        try await reloaded.restore(first)

        let restoredKeys = try await reloaded.closedSessionKeys()
        XCTAssertEqual(restoredKeys, Set([sameSessionInAnotherTask, sameTaskAndSessionInAnotherProject]))
        raw = try readJSONObject(at: stateURL)
        XCTAssertEqual((raw["closed_sessions"] as? [[String: Any]])?.count, 2)
    }

    func testSessionVisibilityPreservesUnknownRootAndEntryFieldsAcrossMutations() async throws {
        let stateURL = byoriHome.appendingPathComponent("manager/session-visibility.json")
        let existingEntry: [String: Any] = [
            "project_id": "projectalpha",
            "task_id": "taskalpha",
            "session_id": "sessionalpha",
            "closed_at": "2026-08-06T01:02:03Z",
            "future_entry_field": ["keep": true],
        ]
        try writeJSONObject([
            "schema_version": 1,
            "future_root_field": ["keep": true],
            "closed_sessions": [existingEntry],
        ], to: stateURL)
        let added = WorkspaceClosedSessionKey(
            projectID: "projectbeta",
            taskID: "taskbeta",
            sessionID: "sessionbeta"
        )
        let store = WorkspaceSessionVisibilityStore(
            home: byoriHome,
            now: { Date(timeIntervalSince1970: 1_700_000_000) }
        )

        try await store.close(added)

        var raw = try readJSONObject(at: stateURL)
        XCTAssertEqual((raw["future_root_field"] as? [String: Any])?["keep"] as? Bool, true)
        var entries = try XCTUnwrap(raw["closed_sessions"] as? [[String: Any]])
        XCTAssertEqual(entries.count, 2)
        XCTAssertTrue(NSDictionary(dictionary: entries[0]).isEqual(to: existingEntry))

        try await store.restore(added)

        raw = try readJSONObject(at: stateURL)
        XCTAssertEqual((raw["future_root_field"] as? [String: Any])?["keep"] as? Bool, true)
        entries = try XCTUnwrap(raw["closed_sessions"] as? [[String: Any]])
        XCTAssertEqual(entries.count, 1)
        XCTAssertTrue(NSDictionary(dictionary: entries[0]).isEqual(to: existingEntry))
    }

    func testSessionVisibilityCorruptionFailsClosedWithoutMutation() async throws {
        let stateURL = byoriHome.appendingPathComponent("manager/session-visibility.json")
        try FileManager.default.createDirectory(
        ...
        let corruptDocuments: [Data] = [
            ...
            try JSONSerialization.data(withJSONObject: [
                "schema_version": 1,
                "closed_sessions": [validEntry, validEntry],
            ]),
            try JSONSerialization.data(withJSONObject: [
                "schema_version": 1,
                "closed_sessions": [[
        ...
        let attempted = WorkspaceClosedSessionKey(
            projectID: "projectbeta",
            taskID: "taskbeta",
            sessionID: "sessionbeta"
        )

        for corruptDocument in corruptDocuments {
            try corruptDocument.write(to: stateURL)
            let originalBytes = try Data(contentsOf: stateURL)
            let store = WorkspaceSessionVisibilityStore(home: byoriHome)

            do {
                try await store.close(attempted)
                XCTFail("Corrupt session visibility metadata must fail closed")
            } catch let error as WorkspaceError {
                guard case .invalidRegistry = error else {
                    return XCTFail("Unexpected error: \(error)")
                }
            ...
            XCTAssertEqual(try Data(contentsOf: stateURL), originalBytes)
        }
    }
