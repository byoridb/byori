...
종료하며 각각 종료 이유를 로그로 남긴다. 종료 시 ByoriDB에서 sign out하지는 않는다. 엔진은
login과 query만 노출하므로 인증된 세션은 서버 측 TTL로 해제된다. 대신 종료 로그에 세션을
남겨서 방치된 세션을 어느 프로세스가 들고 있었는지 추적할 수 있게 한다.
