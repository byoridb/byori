...
- Answer "is this working" before listing the checks it was derived from. A status group of equally weighted label/value pairs makes the reader combine four rows to reach the one conclusion the page exists to state; lead with that conclusion and its consequence, then keep the individual checks as its evidence in the same row grammar.
- Report an installed version next to the published one, and give "up to date" its own state. A version alone cannot say whether it is current, and a failed release lookup must never read as current: name the three states — newest known release, unknown release, unrecorded install — and label the action from the same mapping the version row uses. Install what the page displayed, never a version resolved again behind the button.
- Show a log inside the app rather than revealing its folder. The engine's log is unbounded and carries terminal control sequences, so an editor is handed megabytes to display the last few lines and renders the escapes literally. Present a bounded tail, stripped, one file at a time with each file named for what it holds, and say how much was left out; keep the folder reachable for the whole file.
